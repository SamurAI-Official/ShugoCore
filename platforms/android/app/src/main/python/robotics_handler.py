...
class RoboticsExecutionHandler:
    ...
    def handle(self, decision: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if action_type == "robot_look":
            return self._handle_look(decision)
    ...
    def _handle_gripper(self, decision: Dict[str, Any]) -> Dict[str, Any]:
        """Handle gripper command (accepts grasp/release as param actions)."""
        if not self._check_connection():
            return {"status": "refused", "reason": "ros2 connection lost"}
        params = decision.get("params") or {}
        position = float(params.get("position", 0.0))
        # Verb normalization: grasp -> close, release -> open
        action = str(params.get("action", "")).strip().lower()
        if action == "grasp":
            position = float(params.get("position", 0.5))  # close
        elif action == "release":
            position = float(params.get("position", 0.0))  # open
    ...
    def _handle_look(self, decision: Dict[str, Any]) -> Dict[str, Any]:
        """Handle head pan/tilt (robot_look). Publishes joint targets for
        the neck pan/tilt joints. Safety-bounded: pan/tilt limits are
        enforced before publishing."""
        if not self._check_connection():
            return {"status": "refused", "reason": "ros2 connection lost"}
        if self.is_emergency_stopped():
            return {"status": "refused",
        ...
        # Workspace / joint limits enforced
        pan = max(-1.5, min(1.5, pan))   # ±~86 deg
        tilt = max(-1.0, min(1.0, tilt))  # ±~57 deg
        trajectory = JointTrajectory(
            joint_names=["neck_pan_joint", "neck_tilt_joint"],
            points=[JointTrajectoryPoint(
                positions=[pan, tilt], time_from_start=0.5)],
        )
        self._ros2.publish("/head_controller/follow_joint_trajectory", trajectory)
        self._update_watchdog()
        return {"status": "success", "action": "look", "pan": pan, "tilt": tilt}
