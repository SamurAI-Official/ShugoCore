...
Runs repeated full verification rounds against the live runtime + OS:
    ...
    force-stop -> OS cold-start -> version check -> 10-phase harness ->
...
# bounds a reachable-but-slow run: the full 10-phase set with a measured
...
def run_round(serial, ridx, total_rounds, phases, expect_name, expect_code,
              log):
    """One recursive round: cold-stop -> full 10-phase harness -> health."""
...
def main(argv=None):
    ap = argparse.ArgumentParser(
        description="Recursive loop training: repeated cold-start + full "
                    "10-phase verification rounds per device.")
