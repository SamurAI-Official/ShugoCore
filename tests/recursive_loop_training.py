...
Runs repeated full verification rounds against the live runtime + OS:
    ...
    force-stop -> OS cold-start -> version check -> 15-phase harness ->
...
ALL_PHASES = [
    ...
    "measurement_honesty",
    "ask_user_round_trip",
    # v1.30.6: the layer-split peripheral must start on-device and be reachable
    # before a host can put layers on it.
    "rpc_node_up",
]
...
# bounds a reachable-but-slow run: the full 15-phase set with a measured
...
def run_round(serial, ridx, total_rounds, phases, expect_name, expect_code,
              log):
    """One recursive round: cold-stop -> full 15-phase harness -> health."""
...
def main(argv=None):
    ap = argparse.ArgumentParser(
        description="Recursive loop training: repeated cold-start + full "
                    "15-phase verification rounds per device.")
